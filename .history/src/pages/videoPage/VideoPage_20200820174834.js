...
const VideoPage = () => {
  ...
  useEffect(() => {
    ...
    Axios.get(
      `https://www.googleapis.com/youtube/v3/channels?part=snippet&part=statistics&id=${videoInfo.channelId}&key=${KEY}`
      ...
      <div className="videoPageLeft">
        ...
        <div className="videoPageVideoInfo">
          <p className="videoPageTags">
            {videoInfo.tags?.slice(0, 3).map((v) => (
              <span>#{v} </span>
            ))}
          </p>
          <p className="videoPageTitle">{videoInfo.localized?.title}</p>
          <div className="videoPageDetails">
            <p>25,358 views • {videoInfo.publishedAt?.toDateString()}</p>
