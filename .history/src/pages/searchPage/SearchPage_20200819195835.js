...
const SearchPage = ({ toggle }) => {
  ...
  useEffect(() => {
    Axios.get(
      `https://www.googleapis.com/youtube/v3/search?part=snippet&type=channel&maxResults=10&order=relevance&q=${q}&key=${KEY}`
    ).then((res) => {
      setVideos(res.data.items);
    });

    Axios.get(
      `https://www.googleapis.com/youtube/v3/search?part=snippet&maxResults=1&order=relevance&q=${q}&key=${KEY}`
    ).then((res) => {
