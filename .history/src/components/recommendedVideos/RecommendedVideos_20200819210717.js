...
const RecomendedVideos = ({ toggle }) => {
  ...
  useEffect(() => {
    Axios.get(
      "https://www.googleapis.com/youtube/v3/search?part=snippet&maxResults=16&order=rating&relevanceLanguage=en&key=AIzaSyAXSygRXeYb3IEaZSEhGYbBIrYcAyrKDXE"
  ...
  useEffect(() => {
    videos.map((v) => {
      Axios.get(
        `https://www.googleapis.com/youtube/v3/channels?part=snippet&id=UCBnnls7hGYmkQmuFTDDe10g&key=AIzaSyAXSygRXeYb3IEaZSEhGYbBIrYcAyrKDXE`
      ).then((res) => {
        v = { ...v, ownerPicture: res.data.items[0].snippet.thumbnails };
        setVideos([...videos, v]);
      });
    });
  }, [videos]);
  ...
  return (
    <div className={toggle ? "recomendedVideosBig" : "recomendedVideos"}>
      ...
      <div className="recommendedVideosVideo">
        ...
        />
        <VideoCard
          image="https://images.theconversation.com/files/185535/original/file-20170911-8791-nc85qx.jpeg?ixlib=rb-1.1.0&q=45&auto=format&w=926&fit=clip"
